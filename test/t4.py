    ...
    def _create_keyboard_layout(self):
        ...
        for row_idx, row in enumerate(keyboard_rows):
            ...
            while col_idx < len(row):
                ...
                # 左側 Shift 鍵
                if key == '⇧':
                    key_obj = Key("⇧", 'shift_left', row_idx, col_idx, col_idx + 2, highlight_color='key_pressed')  # 使用 key_pressed 顏色
                    self.key_coordinates['⇧ (L)'] = (row_idx, len(row_buttons))
                    row_buttons.append(('pack', key_obj.get_widget()))
                
                # 右側 Shift 鍵
                elif key == '⇧':
                    key_obj = Key("⇧", 'shift_right', row_idx, col_idx + 24, col_idx + 27, highlight_color='key_pressed')  # 使用 key_pressed 顏色
                    self.key_coordinates['⇧ (R)'] = (row_idx, len(row_buttons))
                    row_buttons.append(('pack', key_obj.get_widget()))
                
                elif key == ' ':
                    key_obj = Key(' ', ' ', row_idx, col_idx, col_idx, highlight_color='key_pressed')  # 讓空格鍵也高亮顯示
                    row_buttons.append(('pack', key_obj.get_widget()))
                elif row[col_idx:col_idx + 9] == "―       ―":  # Detect and handle the full "Space" key
                    key_obj = Key("―       ―", " ", row_idx, col_idx, col_idx + 8, highlight_color='key_pressed')  # 使用 key_pressed 顏色
